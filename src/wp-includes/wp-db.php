...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Prefix de base de dades no vàlid'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>No es pot seleccionar la base de dades</h1>
<p>Hem pogut connectar amb la base de dades del servidor (la qual cosa significa que el nom d\'usuari i contrasenya són correctes) però no hem pogut seleccionar la base de dades <code>%1$s</code>.</p>
<ul>
<li>Esteu segurs que existeix?</li>
<li>Té l\'usuari <code>%2$s</code> permisos per usar la base de dades <code>%1$s</code>?</li>
<li>En alguns sistemes el nom de la base de dades té com a prefix el nom d\'usuari, per la qual cosa seria una cosa com <code>usuari_%1$s</code>. Podria ser aquest el problema?</li>
</ul>
<p>Si no sabeu com configurar una base de dades, hauríeu de <strong>contactar amb el vostre proveïdor</strong>. Si tot falla, podreu trobar ajuda als <a href="http://wordpress.org/support/">fórums de suport del WordPress</a> (en anglès).</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'Error a la base de dades del WordPress %1$s a la consulta %2$s feta per %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'Error a la base de dades del WordPress: %1$s per la consulta %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
'/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, tipus de sortida, int desplaçament) -- El tipus de sortida ha de ser un dels següents: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
