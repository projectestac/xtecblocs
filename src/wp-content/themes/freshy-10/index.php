	...
	<?php elseif (have_posts()) : ?>
		...
		<?php while (have_posts()) : the_post(); ?>
			...
			<div class="post" id="post-<?php the_ID(); ?>">
				...
				<div class="entry">
					<?php the_content('<span class="readmore">'.__('Read the rest of this entry &raquo;', 'freshy').'</span>'); ?>
				</div>
				
				<?php edit_post_link(__('Edit', 'freshy'), '<small class="postmetadata">'.__('Edit', 'freshy').' : ', '</small><br/>'); ?>
				
				<small class="postmetadata"><?php _e('Comments', 'freshy'); ?> : <?php comments_popup_link(__('No Comments &#187;', 'freshy'), __('1 Comment &#187;', 'freshy'), __('% Comments &#187;', 'freshy')); ?></small>
				<br/>
				<small class="postmetadata"><?php _e('Categories', 'freshy'); ?> : <?php the_category(', ') ?></small>
		...
		<p class="navigation">
			<span class="alignleft"><?php next_posts_link(__('&laquo; Previous Entries', 'freshy')) ?></span>
			<span class="alignright"><?php previous_posts_link(__('Next Entries &raquo;', 'freshy')) ?></span>
		</p>
	
	<!-- nothing found -->
	<?php else : ?>
		<div class="post" id="post-none">
			<h2 class="center"><?php _e('Not found', 'freshy'); ?></h2>
			<p class="center"><?php _e("Sorry, but you are looking for something that is not here", 'freshy'); ?></p>
			...
			<?php if(yy_is_home()==true) : ?>
				...
				<?php if(function_exists('c2c_get_recent_posts')) : ?>	
					<div class="highlight_box" id="post-last-works">
						<h2><?php _e('Last works', 'freshy'); ?></h2>
						<ul>
