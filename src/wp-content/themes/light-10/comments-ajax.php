...
if ( empty($post_status) ) {
	...
	fail('The post you are trying to comment on does not curently exist in the database.','light');
} elseif ( 'closed' ==  $post_status ) {
	do_action('comment_closed', $comment_post_ID);
	fail(__('Sorry, comments are closed for this item.','light'));
}
...
if ( $user_ID ) :
	...
	if ( get_option('comment_registration') )
		fail(__('Sorry, you must be logged in to post a comment.','light'));
endif;
...
if ( get_settings('require_name_email') && !$user_ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		fail(__('Error: please fill the required fields (name, email).','light'));
	elseif ( !is_email($comment_author_email))
		fail(__('Error: please enter a valid email address.','light'));
}

if ( '' == $comment_content )
	fail(__('Error: please type a comment.','light'));
