...
class WPTelegram_Helpers {
	...
	public function filter_text_for_parse_mode( $text, $parse_mode ) {
		...
		if ( 'HTML' == $parse_mode ) {
			...
			$text = preg_replace_callback( $pattern, array( __CLASS__, 'get_htmlentities' ), $text );
			...
			if ( 'Markdown' == $parse_mode ) {
				...
				$text = preg_replace_callback( '/\*(.+?)\*/su', array( __CLASS__, 'replace_nested_markdown' ), $text );
			}
		}
	 ...
	 * @return string
	 */
	public function get_htmlentities( $match ) {
