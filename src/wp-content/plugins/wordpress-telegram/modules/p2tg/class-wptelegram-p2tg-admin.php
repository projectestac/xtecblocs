...
class WPTelegram_P2TG_Admin extends WPTelegram_Module_Base {
	...
	private function render_macros() {
		...
		$macro_groups['cf']['html'] = sprintf( __( 'Replace %1$s in %2$s by the name of the Custom Field.', 'wptelegram' ), '<code>custom_field</code>', '<code>{cf:custom_field}</code>' ) . '&nbsp;' . sprintf( __( 'For example %1$s or %2$s  in WooCommerce', 'wptelegram' ), '<code>{cf:_regular_price}</code>', '<code>{cf:_sale_price}</code>' );
	...
	public function create_override_metabox() {
		...
		$box = array(
			...
			'title'         => __( 'WP Telegram P2TG', 'wptelegram' ),
	...
	public function admin_notices() {
		...
		if ( isset( $_GET[ self::$prefix . 'error'] ) && $p2tg_errors = array_filter( (array) get_transient( $transient ) ) ) {
			...
			$html = sprintf( '<b>%s (%s):</b> %s', __( 'WP Telegram', 'wptelegram' ), __( 'Post to Telegram', 'wptelegram' ), __( 'There was some error!', 'wptelegram' ) );
