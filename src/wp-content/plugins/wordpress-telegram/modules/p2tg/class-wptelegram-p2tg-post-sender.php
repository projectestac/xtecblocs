...
class WPTelegram_P2TG_Post_Sender extends WPTelegram_Module_Base {
	...
	public function send_the_post( WP_Post $post, $trigger, $force ) {
		...
		if ( 'future' == self::$post->post_status || 'draft' == self::$post->post_status || ( $ok && $delay = $this->delay_in_posting( $trigger ) ) ) {
	...
	private function security_and_validity_check() {
		...
		if ( $from_web && 'on' === $post_edit_switch && ! $is_cron_rest_or_cli ) {
		...
		$valid_statuses = array( 'publish', 'future', 'draft' );
	...
	private function get_response_text( $template ) {
		...
		if ( 'Markdown' == $parse_mode ) {
			$macro_values = array_map( array( WPTG()->helpers,'esc_markdown' ), $macro_values );
		}
	...
	private function send_responses() {
		...
		$this->tg_api = new WPTelegram_Bot_API( $this->bot_token );

		$api_responses = array();

		do_action( 'wptelegram_p2tg_before_send_responses', $this->responses, $api_responses, self::$post, $this->options, $this->tg_api );
		...
		$channels = (array) apply_filters( 'wptelegram_p2tg_send_to_channels', $channels, $this->responses, self::$post, $this->options, $this->tg_api );
		...
		foreach ( $channels as $channel ) {
			...
			foreach ( $this->responses as $response ) {
				...
				if ( $message_as_reply && $this->tg_api->is_success( $res ) ) {
		...
		do_action( 'wptelegram_p2tg_after_send_responses', $this->responses, $api_responses, self::$post, $this->options, $this->tg_api );
