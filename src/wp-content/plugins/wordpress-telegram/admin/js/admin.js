(function( $, l10n ) {
	...
	wptelegram.bot_api = new Proxy( new wptelegram.bot_api_client(), {
		get : function( client, prop ) {
			if ( 'undefined' === typeof client[prop] ) {
				return function( api_params, response_handler, additional_params, btn ) {
	...
	wptelegram.utils.send_test_message = function( chat_ids, chat_table, btn ){
		...
		for ( var i = 0; i < chat_ids.length; i++ ) {
			if ( chat_ids[i] ) {
				wptelegram.bot_api.sendMessage( {
					chat_id:chat_ids[i],text:new_text}, wptelegram.utils.handle_send_test_message, chat_table, btn );
			}
		}
	};
